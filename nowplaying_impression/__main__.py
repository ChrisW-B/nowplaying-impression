...
def draw_top_tracks(tracks):
    bg = Image.new("RGBA", (inky.WIDTH, inky.HEIGHT), (255, 255, 255, 255))
    draw = ImageDraw.Draw(bg)

    album_block_size = int(inky.HEIGHT / 3)

    for i, album in enumerate(tracks):
        x = i % 4
        y = math.floor(i / 4)
        img = Image.open(requests.get(
            album['album_image'], stream=True).raw).convert("RGB")
        ...
        draw.text(((x * album_block_size) + 2, (y * album_block_size) + 2), str(i + 1),
                  font=index_font, fill='White', stroke_width=2, stroke_fill='Black')
...
def main():
    ...
    if now_playing and now_playing['is_playing']:
        ...
        most_played = sp.current_user_top_tracks(
            limit=12, time_range='short_term')
        draw_top_tracks(list(map(simplify_track, most_played['items'])))
